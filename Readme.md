...
Simply instantiate the PDO\_Wrapper class:
			...
			\<?php  
				require\_once 'db.php'; // This initializes $db

			// If you need another connection:  
			// $db\_secondary \= new PDO\_Wrapper('pgsql', 'another\_host', 'another\_db', 'user', 'pass', \[\], true);  
			?\>
...
#### **Insert Single Record**
			...
			$user\_data \= \[  
				...
				'status' \=\> 'active',  
			\];  
			$inserted\_id \= $db-\>insert('users', $user\_data);  
			if ($inserted\_id) {  
				echo "User inserted with ID: " . $inserted\_id;  
			}  
			?\>
...
#### **Insert Multiple Records**
			...
			\];  
			$affected\_rows \= $db-\>insert\_multi('products', $products\_data);  
			echo "Inserted {$affected\_rows} products.";  
			?\>
